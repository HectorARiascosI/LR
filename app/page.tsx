...
import { EscanorSprite, MerlinSprite } from "@/components/CharacterSprites";

const RainWorld = dynamic(() => import("@/components/RainWorld"), {
  ssr: false,
  loading: () => null,
});

// ─── MÚSICA — piano melancólico con lluvia ────────────────────────────────────
function usePianoMusic() {
  ...
  // Escala menor natural — melancólica, acorde con lluvia
  ...
  const schedule = useCallback(() => {
    ...
    while (nextNoteAt.current < ctx.currentTime + LOOKAHEAD) {
      const idx = PATTERN[noteIdx.current % PATTERN.length];
      const freq = MELODY[idx];
      playNote(freq, nextNoteAt.current, 1.4, 0.09);
      // Bajo cada 4 notas
      if (noteIdx.current % 4 === 0) {
        playNote(freq * 0.25, nextNoteAt.current, 2.2, 0.07);
      }
      nextNoteAt.current += STEP;
      noteIdx.current++;
    }
  ...
  const stop = useCallback(() => {
    ...
    if (masterRef.current) {
      ...
      setTimeout(() => {
        if (masterRef.current) masterRef.current.gain.value = 0.55;
      }, 600);
    }
  }, []);
...
const SECTIONS = [
  ...
  <div key="3" className="text-block">
    ...
    <FadeIn delay={0.85}><p className="t-body" style={{ marginTop: "1.2rem" }}>Como Merlin ante Escanor —<br />una presencia que lo cambia todo<br />sin siquiera intentarlo.</p></FadeIn>
  </div>,
  // 4
  ...
  <div key="6" className="text-block">
    ...
    <FadeIn delay={0.95}><p className="t-verse c-gold glow-gold">Como Escanor al sol —<br />tu presencia me hace brillar<br />más de lo que creía posible.</p></FadeIn>
  ...
  return (
    <>
      ...
      <div style={{ position: "fixed", inset: 0, zIndex: 0 }}>
        <RainWorld scrollRef={scrollRef as React.RefObject<number>} />
      </div>

      {/* Personajes — aparecen desde sección 3, posicionados sobre el suelo 3D */}
      <AnimatePresence>
        {showChars && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            <EscanorSprite section={section} />
            <MerlinSprite  section={section} />
          </motion.div>
        )}
      </AnimatePresence>
